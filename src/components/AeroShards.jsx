...
const advanceHold = (hold, elapsed, disabled) => {
  ...
  if (Math.abs(hold.amount - target) < 0.0001 && Math.abs(hold.velocity) < 0.001) { hold.amount = target; hold.velocity = 0; }
  if (hold.amount > 0) hold.phase += elapsed * (0.35 + hold.amount * 0.5);
};

const resetPointerMotion = pointer => { pointer.velocity ??= [0, 0]; pointer.velocity[0] = 0; pointer.velocity[1] = 0; pointer.presenceVelocity = 0; };
...
const advancePointer = (pointer, elapsed) => {
  ...
  if (Math.abs(pointer.presence - presenceTarget) < 0.001 && Math.abs(pointer.presenceVelocity) < 0.01) { pointer.presence = presenceTarget; pointer.presenceVelocity = 0; }
};
...
fn centerPath(seedPhase: f32, distance: f32, aspect: f32) -> PathSample {
  ...
  var sample: PathSample;
  sample.position = vec3f(cos(angle) * radius * aspect, sin(angle) * radius, sin(t * pi * 2.0) * 0.16);
  sample.tangent = safeNormalize(derivative);
  sample.phase = phase;
  return sample;
}
fn fullPath(seedPhase: f32, distance: f32, aspect: f32) -> PathSample {
  ...
  var sample: PathSample;
  sample.position = vec3f(mix(-aspect * 1.22, aspect * 1.22, t), sin((t * 1.72 - 0.2) * pi) * 0.54 + sin(t * pi * 3.0) * 0.12, cos(t * pi * 2.0 - 0.7) * 0.22);
  sample.tangent = safeNormalize(derivative);
  sample.phase = phase;
  return sample;
}
fn mobilePath(seedPhase: f32, distance: f32, aspect: f32) -> PathSample {
  ...
  var sample: PathSample;
  sample.position = vec3f(mix(-aspect * 1.28, aspect * 1.28, t), -0.86 + sin(t * pi) * 0.28 + sin(t * pi * 3.0) * 0.06, cos(t * pi * 2.0) * 0.16);
  sample.tangent = safeNormalize(derivative);
  sample.phase = phase;
  return sample;
}
fn weightedPath(seedPhase: f32, phaseOffset: f32, aspect: f32, weights: vec4f) -> PathSample {
  ...
  result.position = vec3f(0.0); result.tangent = vec3f(0.0); result.phase = phase;
  if (aspect < 0.82) {
...
fn softbox(direction: vec3f, center: vec2f, size: vec2f) -> f32 {
  let q = abs((direction.xy - center) / size); let q2 = q * q; let q4 = q2 * q2;
...
fn vs_main(@builtin(vertex_index) vertexIndex: u32, @builtin(instance_index) instanceIndex: u32) -> VertexOut {
  ...
  let rollSin = sin(roll); let rollCos = cos(roll);
  ...
  let world = renderPosition + direction * local.y * lengthScale + bankedSide * local.x * width + bankedFacing * local.z * width;
...
const BLOOM_SHADER = `
struct PostParams {
  viewport: vec4f, bloomInfo: vec4f, finishing: vec4f, background: vec4f, temporal: vec4f, tint: vec4f,
}
...
fn visibleResidual(uv: vec2f) -> vec4f {
  ...
  let threshold = post.bloomInfo.z; let knee = post.bloomInfo.w;
...
const createRenderGraph = (gpu, outputSize, bloomSize = resolveBloomSize(outputSize)) => {
  const viewParams = uniforms(gpu, {
    ...
    light: [-0.321, 0.49, 0.845, 0], environment: [0, 0, 0, 0],
  ...
  return {
    viewParams, postParams, shardDraw, sceneTarget, bloomTarget, bloomScratchTarget,
    bloomEffect, blurParamsX, blurParamsY, bloomBlurX, bloomBlurY, finishEffect,
    styleTarget, asciiTarget, styleParams,
    styleSignature: ''
  };
};
...
export default function AeroShards({
  ...
  settingsRef.current = {
    ...
    glow: resolvedMaterial.glow * resolvedGlow, edgeSoftness: resolvedEdgeSoftness,
    ...
    holdToGather, paused,
    signature: [backgroundColor, shardColor, accentColor, placement, flow, material, detail, effectProp,
      ...
      rippleIntensity, holdToGather, paused].join('|')
  };
  ...
  useEffect(() => {
    ...
    const handlePointerMove = event => {
      ...
      if (!next) { const pointer = pointerRef.current; if (pointer.active || pointer.presence > 0) deactivatePointer(); return; }
    ...
    visibilityObserver = new IntersectionObserver(entries => {
      ...
      if (visible) { resumePending = true; } else {
        interactionDeadline = 0; settlingDeadline = 0;
        const pointer = pointerRef.current; pointer.active = 0; pointer.presence = 0;
        holdRef.current.pointerId = null; resetPointerMotion(pointer);
      }
      wakeRenderer();
    ...
    void (async () => {
      try {
        ...
        const formationState = createFormation(settingsRef.current.flow);
        ...
        const resolveFrameState = now => {
          ...
          if (now < interactionDeadline || pointerTransitioning || ripplesRef.current.some(r => r.strength > 0) || layoutTransitioning || holdRef.current.pointerId !== null || holdRef.current.amount > 0 || formationState.weights[settingsRef.current.flow] < 1 || Math.abs(settingsRef.current.scale - renderScale) > 0.001) return FRAME_STATES.interactive;
        ...
        const resizePostTargets = (qualityLevel = appliedBloomLevel) => {
          ...
          graph.bloomTarget.resize(bloomSize); graph.bloomScratchTarget.resize(bloomSize);
          graph.blurParamsX.set({ direction: [1/bloomSize[0], 0, 0, 0] });
          graph.blurParamsY.set({ direction: [0, 1/bloomSize[1], 0, 0] });
          graph.postParams.set({ viewport: [width, height, 1/width, 1/height], bloomInfo: [1/bloomSize[0], 1/bloomSize[1], 0.2, 0.12] });
        };
        ...
        const setRuntimeQuality = (nextLevel, now, frameState) => {
          ...
          runtimeQualityLevel = clampedLevel; pressureStartedAt = 0; stableStartedAt = now; lastQualityChange = now;
        ...
        const renderFrame = currentFrame => {
          ...
          if (frozen) { renderScale = settings.scale; } else { renderScale += (settings.scale - renderScale) * (1 - Math.exp(-elapsed * 12)); }
          ...
          if (!frozen) {
            ...
            travelPhase = (travelPhase + (elapsed * settings.speed * 0.34) / resolvePathLength(travelAspect, layoutWeights)) % 1;
          }

          const pointer = pointerRef.current;
          advanceFormation(formationState, settings.flow, elapsed, settings.transitionDuration, frozen);
          ...
          const activeCount = Math.max(700, Math.round(preset.count * settings.detailCount * runtimeQuality.countScale));
          ...
          const lightX = -0.38 + pointerShiftX; const lightY = 0.58 + pointerShiftY;
          const lightLength = Math.hypot(lightX, lightY, 1);
          const interactionSign = settings.interaction === INTERACTIONS.attract ? 1 : -1;
          const interactionPresence = settings.interaction === INTERACTIONS.none ? 0 : pointer.presence * settings.interactionStrength * interactionSign * (1 - holdRef.current.amount);
          ...
          const rippleUniforms = ripplesRef.current.map(ripple => [(ripple.origin[0]*2-1)*aspect*inverseScale, (1-ripple.origin[1]*2)*inverseScale, ripple.age, ripple.strength*settings.interactionStrength*settings.rippleIntensity*inverseScale]);
          ...
          graph.viewParams.set({
            ...
            formation: formationState.weights,
            gather: [pointerWorldX, pointerWorldY, holdRef.current.amount, holdRef.current.phase],
            pointer: [pointerWorldX, pointerWorldY, settings.interactionRadius * 2 * inverseScale, interactionPresence * inverseScale],
            shock: rippleUniforms[0], shockB: rippleUniforms[1], shockC: rippleUniforms[2], shockD: rippleUniforms[3],
        ...
        const scheduleSleep = targetTimestamp => {
          ...
          timeoutId = window.setTimeout(() => { timeoutId = 0; scheduleRaf(); }, delay);
        };
        ...
        const scheduleFrame = timestamp => {
          ...
          if (previousRafTimestamp) {
            ...
            if (refreshSample > 3 && refreshSample < 35) {
              ...
              for (let index = 1; index < refreshSampleCount; index += 1) refreshInterval = Math.min(refreshInterval, refreshSamples[index]);
            }
          }
          ...
          if (runtimeQualityLevel < RUNTIME_QUALITY.length - 1 && qualityCooldownComplete && sustainedPressure) { setRuntimeQuality(runtimeQualityLevel + 1, monitorNow, frameState); }
